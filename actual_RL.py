...
def save_result(scenario_id, strategy, results, information):
    ...
    with open('results/results.txt', 'a') as f:
        ...
        print_string = strategy + ',' + str(scenario_id) + ',' + str(information[0]) + ',' + str(information[1]) + ',' + str(results[0]) + ',' + str(results[1]) + ',' + str(results[2]) + ',' + str(results[3]) + ',' + str(results[4]) + ',' + str(results[5]) + '\n'
...
def run_env(run_number, no_trucks = 3, no_clusters = 6, no_drones = 3, no_customers = 60, p = [1], load = False, load_file = None, strategy = 'next_closest', save_state=False, drone_capacity = 2):
    ...
    while True:
        ...
        if done[0]:
            ...
            return (steps, drone_travel_distance, utilization, total_package_waiting_time, total_customer_waiting_time)
...
#     run_env(i, no_trucks, None, no_drones, None, p, load=True, load_file = filename, strategy='farthest_package_first', save_state=False, drone_capacity = drone_capacity)


#Before we begin the simulation we want to initialize the csv file which will store the results

with open('results/results.txt', 'w') as f:
    f.write('strategy,scenario_id,drone_capacity,no_customers,total_time,A,drone_travel_distance,utilization,avg_package_wait_time,avg_customer_wait_time\n')
...
for drone_capacity in drone_capacity_values:
    ...
    for i in range(20):
        ...
        filename = path + 'saved_state' + str(i) + '.txt'
        steps, drone_travel_distance, utilization, package_wait_time, customer_wait_time = run_env(None, no_trucks, None, no_drones, None, p, load=True, load_file=filename, strategy=strategy, save_state=False, drone_capacity = drone_capacity)    
        f = open(filename).readlines()
        no_customers = len(f) - 1
        no_packages = sum([int(i.split(',')[-1]) for i in f[1:]])
        save_result(i, strategy, (steps[0], steps[1], drone_travel_distance, utilization , package_wait_time / no_packages, customer_wait_time / no_customers), (drone_capacity, no_customers))
...
#         save_result(i, strategy, (steps[0], steps[1], drone_travel_distance, utilization , package_wait_time, customer_wait_time / no_customers), (drone_capacity, no_customers))
